<template>
  <div id="miDiv" class="TrimWheel" :class="{ grabbing: isDragging }" @mousedown="startDrag" @mouseup="stopDrag" @mousemove="handleDrag">
    <img ref="image" src="src\assets\rectangle.png" :style="{ transform: `translateY(${currentTranslationY}px)`, width: '150px', height: '30px' }" draggable="false" />
    <div class="title">{{ currentTranslationY }}</div>
  </div>
</template>
  ...
  methods: {
    ...
    handleDrag(event) {
      if (this.isDragging) {
        ...
        this.currentTranslationY = Math.max(minTranslationY, Math.min(maxTranslationY, nextTranslationY)); // Ajustar dentro del rango
